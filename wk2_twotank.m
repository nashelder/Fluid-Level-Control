...
load('openloopdistesp.mat');
plot(TwoTank(:,1), TwoTank(:,3))

hold on;

load('wk2_TwoTank.mat');
TwoTank(:,1) = TwoTank(:,1)-26;
plot(TwoTank(26:856,1), TwoTank(26:856,3))

hold on;

plot(tout,height_bottom+7.75);

grid on;
title ('Step Response from Steady State'); % You must have graph title
xlabel ('Time [sec]'); % and axis labels
ylabel ('Height Change [in.]');
xlim([0 600]);
legend('Open Loop Experimental','Model','Closed Loop Experimental');
%load('openloopresp2.mat');
